...
void http_client(const std::string &url, const int &port, tools::Method method)
{
    if (method == tools::Method::GET)
    {
        std::unique_ptr<tools::HTTPClient> httpClient(new tools::HTTPClient());

        std::string data = httpClient->get(url, port);
        std::cout << data << std::endl;

    } else if (method == tools::Method::POST)
    {
        ...
        std::string cert = httpClient->post(url, formParamStr, port);
...
int main(int argc, char* argv[])
{
    ...
    } else if (args.client)
    {
        if (args.get)
        {
            http_client(args.url, args.port, tools::Method::GET);
        } else if (args.post)
        {
            http_client(args.url, args.port, tools::Method::POST);
        } else
        {
            std::cerr << "Only GET and POST are supported for now!" << std::endl;
        }

    } else
    {
        std::cerr << "Wrong usage of the arguments!" << std::endl;
    }
